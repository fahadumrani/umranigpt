...
window.OllamaService = (() => {
  ...
  const getBase = () => normaliseUrl(window.AppStorage.getOllamaUrl());

  const endpoint = (path) => {
    const base = getBase();
    if (!base) throw new Error('Ollama URL not configured. Go to Settings to set it.');
    return base + path;
  };
